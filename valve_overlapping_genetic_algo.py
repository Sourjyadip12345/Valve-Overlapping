...
def valve_overlapping(input_valves=None,population_size=500,gen_theshold=50,cycles=None ):
    ...
    for _ in range(4):    
            ...
            def flatten_tuple_list(lst):
                ...
                for item in lst:
                    if isinstance(item, tuple) or isinstance(item, list):
                        return [sum(column) for column in zip(*lst)]
                    else:
                        flattened.append(item)
                return flattened
            
            FINAL_RESULT_SCORE.append(max(flatten_tuple_list(C)))
            FINAL_RESULT.append([B,C,D,E])
        ...
        def fitness_stddev(chromosome):
            ...
            return fitness#*max(sum_timings)   #fitness
        ...
        def scheduled_valves(schedule,input_valves):
            ...
            for i in range(len(input_valves)):
                to_append=(input_valves[i][1]*[1]+input_valves[i][0]*[0])*int(LCM/(input_valves[i][1]+input_valves[i][0]))
                sum_timings.append(to_append[-schedule[i]:]+to_append[:-schedule[i]])
